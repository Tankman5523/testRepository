...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * -request : 서버로 요청할때의 정보들이 담겨있다. (요청시 전달값,요청 전송 방식 등등)
		 * -response : 요청에 대해 응답에 필요한 객체
		 * 
		 * GET / POST 방식 차이
		 * -Get : 사용자가 입력한 값이 url에 노출되고 데이터의 길이제한이 있으며 즐겨찾기를 할 수 있다.
		 * -Post : 사용자가 입력한 값이 url에 노출되지 않고 데이터의 길이제한이 없으며 즐겨찾기를 할 수 없다.
		 * 
		 * */
		
		//1)POST 요청일 경우 전달값에 한글이 포함되어 있다면 encoding설정을 해야한다.
...
//		System.out.println("조회된 Member : "+m);
		...
		/*
		 * 응답 페이지에 전달할 값이 있다면 값을 어딘가엔 담아서 전달해야한다.(이때 담아줄 수 있는 내장객체 4가지)
		 * -servlet scope
		 * 1)application : application에 담은 데이터는 웹 애플리케이션 전역에서 꺼내 쓸 수 있다.
		 * 2)session : session 에 담은 데이터는 모든 jsp와 servlet에서 꺼내 쓸 수 있다.
		 * 				한번 담은 데이터는 직접 지우기 전까지,서버가 멈추기 전까지,브라우저가 종료되기 전까지는 사용가능하다.
		 * 3)request : request 에 담은 데이터 해당 request를 포워딩한(위임한) 응답jsp에서만 꺼내쓸 수 있다.
		...
		if(loginUser ==null) {//로그인 실패(에러페이지로 응답해보기)
			...
			//* 세션은 싱글톤 방식으로 작동하기 때문에 객체를 받아올때마다 이전에 넣은 데이터를 담고있다.
			...
			//응답방식 2가지
			//1. 위임(포워딩) - 요청을 유지한 채로 위임하는 방법 (url에 기존 주소가 남아있게된다.)
...
//			System.out.println(before);
			...
			response.sendRedirect(before); //로그인 이전에 사용하던 위치로 이동

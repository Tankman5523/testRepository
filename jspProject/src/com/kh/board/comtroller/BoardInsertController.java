	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * 사용자가 올린 파일명은 그대로 해당 폴더에 업로드하지 않는게 일반적이다. 
			 * - 같은 파일명이 있을 경우 덮어씌워질 경우도 있음
			 * - 한글 / 특수문자가 포함된 파일명의 경우는 업로드되는 서버에 따라 오류를 발생시킬 여지가 있기 때문
			 * 
			 ...
			 * rename 메소드를 정의하면 해당 작업을 할 수 있다.
			 * 
			 * */
...
//			String file = multiRequest.getParameter("upfile"); //*기존하고 차이
			...
			//첨부파일이 있을 경우 원본명 반환 , 없는경우 null반환
			...
			if(result>0) {
				...
				//실패시에는 업로드된 파일을 지워주는 작업이 필요하다.(게시글은 없는데 업로드 파일이 자원을 쓰고있음)
				if(at!=null) {//넘어온 파일이 있어서 객체가 생성되었다면.
					//해당 경로 파일 잡아서 File객체 생성 후 delete메소드로 파일 삭제 작업
